...
import { encodeToken, fetchApi } from "@/useJiraAPI.js";
  ...
  .command(
    ...
    async (argv) => {
      ...
      if (config) {
        ...
        const key = "TnthkyhfHUIqYZViRWfk268F";
        const token = encodeToken(mail, key);
        const a = await fetchApi(token, "aexoldev.atlassian.net");
        // const a = await getCloudIdByHostName(token, "aexoldev.atlassian.net");
        console.log("TO JEST ODPOWIEDX");
        console.log(a);
